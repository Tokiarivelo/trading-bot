...
/** Shared chrome for the Bots hub: header, stats grid, and tab nav. Each tab
 * is a real nested route (/bots/deployments, /bots/factory, /bots/library,
 * /bots/drafts) rendered as `children` rather than client-side tab state, so
 * each screen is linkable/bookmarkable and back/forward navigation works. */
...
export default function BotsLayout({ children }: { children: React.ReactNode }) {
  ...
  const activeTab = TABS.find((t) => t.id === activeSegment) ?? TABS[0];
      ...
      <main className="min-h-0 flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1 px-1">
            <h2 className="text-base font-bold text-ink">{activeTab.label.split(" ").slice(1).join(" ")}</h2>
            <p className="text-xs text-ink-muted">{activeTab.description}</p>
          </div>
