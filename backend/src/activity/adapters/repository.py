...
class ActivityLogRepository:
    ...
    def save(self, *, created_at: int, level: str, logger: str, message: str) -> None:
        with self._session_factory() as session:
            session.add(LogRow(created_at=created_at, level=level, logger=logger, message=message))
            session.commit()
    ...
    def delete_by_ids(self, ids: list[int]) -> int:
        ...
        if not ids:
            return 0
        with self._session_factory() as session:
            result = session.execute(delete(LogRow).where(LogRow.id.in_(ids)))
            session.commit()
