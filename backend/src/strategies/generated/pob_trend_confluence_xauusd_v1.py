...
def _classify_bars(
    ...
    so `PobTrendConfluenceXauusd._detect_zones_cached` can reuse it — keep
    the two in sync (same rule as the sibling `pob_snd_zones_xauusd_v1`)."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    segments in `PobTrendConfluenceXauusd._detect_zones_cached`) — see
...
def _detect_zones(df: pd.DataFrame, atr: pd.Series, params: dict) -> list[dict]:
    ...
    the stateless ground truth. `PobTrendConfluenceXauusd.evaluate()` uses
    the incremental, cache-backed `_detect_zones_cached` instead for its
    own (repeatedly-called, sliding-window) zone_frame; this function stays
...
class PobTrendConfluenceXauusd:
    ...
    def _detect_zones_cached(
        ...
        call — proven by
        `tests/unit/strategies/test_pob_trend_confluence_xauusd.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(zone_frame)
        valid_atr = atr.dropna()
        if valid_atr.empty:
