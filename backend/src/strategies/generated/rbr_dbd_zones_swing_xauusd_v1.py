...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesSwingXauusd._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesSwingXauusd._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    in `RbrDbdZonesSwingXauusd._detect_zones_cached`). A from-scratch
...
def _build_zones_from_runs(
    ...
    both the full recompute and `RbrDbdZonesSwingXauusd._detect_zones_cached`
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesSwingXauusd` uses the
...
class RbrDbdZonesSwingXauusd:
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_rbr_dbd_zones_swing_xauusd.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
