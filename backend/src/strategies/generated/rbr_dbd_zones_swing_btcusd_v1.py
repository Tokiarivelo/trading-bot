"""RBR/DBR demand-zone swing strategy for BTCUSD (M15 entries,
H1+H4 trend), long-only.

Same mechanics and same tuned recipe as `rbr_dbd_zones_swing_xauusd_v1.py`
(see that module for the full derivation), ported to BTCUSD rather than
...
These are the XAUUSD-tuned parameter values, not re-derived for BTCUSD's own
volatility/tick behavior — validate with a BTCUSD backtest before trusting
...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesSwingBtcusd._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesSwingBtcusd._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    in `RbrDbdZonesSwingBtcusd._detect_zones_cached`). A from-scratch
...
def _build_zones_from_runs(
    ...
    both the full recompute and `RbrDbdZonesSwingBtcusd._detect_zones_cached`
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesSwingBtcusd` uses the
...
class RbrDbdZonesSwingBtcusd:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="rbr_dbd_zones_swing_btcusd",
            version=1,
            symbols=("BTCUSD",),
            entry_timeframe="M15",
            ...
            params={
                "atr_period": 14,
                # Per-symbol retune 2026-07-19 (2026-05:2026-07 sweep):
                # PF 0.37->1.15 on 6 trades — thin M15 history, low confidence
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_rbr_dbd_zones_swing_btcusd.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
