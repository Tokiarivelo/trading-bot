"""RBR/DBD/RBD/DBR zone-retest scalp strategy for BTCUSD (M5 entries, M15
trend filter) -- fills the timeframe gap between `rbr_dbd_zones_scalp_btcusd`
(M1 entries, M5 trend filter) and `rbr_dbd_zones_swing_btcusd` (M15 entries,
H1/H4 confirmation). No M5-entry BTCUSD family existed before this one.

Same mechanics as `rbr_dbd_zones_scalp_btcusd_v2.py` (see that module for the
full derivation) with the entry/confirmation/trend timeframes shifted up one
notch (M1->M5, M5->M15), the same shift the M1 scalp already applies relative
to the M15 swing family. Two differences from a pure timeframe relabel:

  - `retest_max_age_bars` widened from 2 to 3: on M1 a stale retest is ~2
    minutes old; the M5-equivalent staleness window is proportionally wider,
    not the same *bar count*, since each bar covers 5x the wall-clock time.
  - `confirm_lookback` kept at 6 M15 bars (1.5h) rather than 6 M5 bars (30m)
    on the confirmation timeframe -- mirrors the M1 scalp's own 6-M5-bar
    (30-minute) window scaled up by the same one-notch shift.

Everything else (zone/swing detection, engulfing-only entry confirmation,
SL = base rectangle height, TP = nearest unmitigated swing with a capped RR
fallback, `zone_lookback_bars=200` to fit the engine's context-bar cap) is
identical to the M1 family, including the additive RSI(14)/EMA(20,50)/
volume-ratio confluence TP tiering from `rbr_dbd_zones_scalp_btcusd_v2.py`
-- built in from the start here rather than added in a later version, since
the M1 study already established that RSI/EMA/volume (not ADX) are the
confluence signals that hold up for this strategy family on BTCUSD. Those
same three votes, recomputed on the M5 entry timeframe, only ever widen the
TP (tp_multiplier >= 1.0), never gate the entry -- so trade count depends
only on the (currently untested for M5) core trigger, never on confluence.

These are the M1-family-tuned parameter values carried over as a starting
point for the M5 timeframe, not independently re-derived -- validate with a
BTCUSD M5 backtest before trusting them, and retune (particularly
`base_body_atr_mult`/`leg_travel_atr_mult`/`max_base_atr_mult`/
`entry_max_dist_atr_mult`, the ones most sensitive to how much a single
candle's range differs between M1 and M5) if the results don't transfer.

No live/backtest track record yet -- validate with `/backtest/run` before
activating. Sandbox-safe: only `numpy`/`pandas` -- no I/O, no broker access.
"""
...
TP_MULT_BASE = 1.0  # 0-1 of 3 confluence votes -- the strategy's own baseline TP, never shrunk
...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesScalpM5Btcusd._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesScalpM5Btcusd._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    segments in `RbrDbdZonesScalpM5Btcusd._detect_zones_cached`). A
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesScalpM5Btcusd` uses the
...
def _confluence_votes(
    ...
    all computed on the entry timeframe (M5) closes -- distinct from the
    existing `_trend_direction` M15 gate. NaN (not enough warmup) degrades
    to "not confirmed" for that vote -- never raises, never skips the
    trade, since this whole path is additive-only."""
...
class RbrDbdZonesScalpM5Btcusd:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="rbr_dbd_zones_scalp_m5_btcusd",
            version=1,
            symbols=("BTCUSD",),
            entry_timeframe="M5",
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_rbr_dbd_zones_scalp_m5_btcusd.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
    ...
    def evaluate(self, ctx: MarketContext) -> Signal | None:
        ...
        # both underperformed engulfing badly on the M1 study), then follow
        # the trend.
        ...
        # setups, never shrinks it below tp_mult_base=1.0, so this can only
        # make min_rr_floor easier to clear, never harder.
