...
def _classify_bars(
    ...
    keep it in sync with `TrendStructureV4._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `TrendStructureV4._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    in `TrendStructureV4._detect_zones_cached`). A from-scratch
...
def _build_zones_from_runs(
    ...
    both the full recompute and `TrendStructureV4._detect_zones_cached` share
...
def _detect_zones(
    ...
    this is the stateless ground truth. `TrendStructureV4` uses the
...
class TrendStructureV4:
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_trend_structure_v4.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
