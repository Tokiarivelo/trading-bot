"""RBR/DBD/RBD/DBR zone-retest scalp strategy for BTCUSD (M1 entries, M5
trend filter) -- v2 adds an additive RSI(14)/EMA(20,50)/tick-volume
confluence score on the entry timeframe that only ever *widens* the TP
target, never gates the entry.

v1's entry trigger (engulfing-only confirmation candle, RBR/DBD/RBD/DBR
zone-retest, M5 EMA trend filter, M5 engulf/body-candle MTF confirmation)
and SL (base rectangle height, floored at `min_sl_atr_mult` x ATR) are
unchanged byte-for-byte here -- same reasoning as `trend_structure_v7`
(this repo's other confluence-refinement pass, on a different strategy
family for XAUUSD/XAGUSD/BTCUSD): a hard veto/filter necessarily trades
less often than the entries it's filtering, so any new gate would regress
trade count even if the excluded bucket is individually weaker. v2 avoids
that by only ever extending the TP distance for higher-conviction setups.

A feature-correlation study against v1's own 295-trade BTCUSD 2026-06:2026-07
backtest (76.9% win, PF 4.10, avg_r +0.663, ending balance $24,649.77 from a
$10k start) found a much sharper split than the same study on XAUUSD's
`trend_structure_v2`:

  - RSI(14) alignment (>50 for a buy / <50 for a sell) at the entry bar:
    145 "aligned" trades won 92.4% of the time, +219.68R total (avg
    +1.515R); 150 "unaligned" trades won only 62.0% of the time and were
    net *negative* at -24.07R total (avg -0.160R) -- a real, large losing
    bucket (over half the sample) despite v1's already-strong aggregate
    numbers.
  - EMA(20)/EMA(50) alignment **computed on the entry timeframe (M1)** --
    distinct from the existing `_trend_direction` M5 trend-filter gate,
    which only checks up/down, not degree of alignment: 179 aligned trades
    averaged +0.970R vs 116 misaligned at +0.190R -- still net positive
    either way, but a real gap.
  - ADX(14) > 25: essentially no signal here (avg +0.684R vs +0.640R,
    correlation +0.023) -- unlike the weak-but-real split it showed on
    XAUUSD's trend_structure_v2, it does not transfer to this strategy/
    symbol. Dropped in favor of the indicator that actually discriminates
    for BTCUSD's own trades (see next point) rather than assuming the
    XAUUSD study's indicator choices transfer unchanged.
  - Tick-volume ratio vs its own 20-bar average: *positively* correlated
    here (corr +0.217, the strongest continuous correlation found -- above-
    average-volume entries averaged +0.878R vs +0.429R below-average) --
    the **opposite** relationship from what the same test found on XAUUSD's
    trend_structure_v2 (there it was negative and dropped). Kept here as
    the third confluence vote precisely because it holds up on this
    strategy's own data, not the other symbol's.
  - RSI aligned & EMA aligned combined: 119 trades at 92.4% win / +1.538R
    avg vs 176 at 66.5% win / +0.071R avg (barely positive, not negative --
    the cleanest split found).

Because trade count must not regress, RSI/EMA(M1)/volume confluence is used
*additively only* -- it multiplies whichever TP source v1 already picked
(nearest unmitigated swing, or the `fallback_rr` distance when none exists)
by a factor >= 1.0, never below it, and never vetoes a signal:

  - all 3 confirm (RSI, EMA(M1), volume all aligned/above-average):
    tp_multiplier = 1.4
  - exactly 2 of 3 confirm: tp_multiplier = 1.2
  - 0 or 1 of 3 confirm (including the -24.07R RSI-unaligned bucket):
    tp_multiplier = 1.0, i.e. identical to v1 -- unchanged, not shrunk

A tp_multiplier >= 1.0 can only make the `min_rr_floor` check below easier
to pass than v1's already-proven margin, never harder, so this cannot
introduce a new veto path. Indicator math failing to warm up (NaN) degrades
to "not confirmed" for that vote, never to skipping the trade.

No live/backtest track record yet for v2 itself -- validate with
...
# BTCUSD point size (configs/symbols/btcusd.yaml) — converts ctx.spread_points
...
POINT_VALUE = 0.01
CONFLUENCE_RSI_PERIOD = 14
CONFLUENCE_EMA_FAST = 20
CONFLUENCE_EMA_SLOW = 50
TP_MULT_TIER3 = 1.4  # all 3 confluence votes aligned
TP_MULT_TIER2 = 1.2  # exactly 2 of 3 confluence votes aligned
TP_MULT_BASE = 1.0  # 0-1 of 3 confluence votes -- identical to v1, never shrunk
...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesScalpBtcusdV2._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesScalpBtcusdV2._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    segments in `RbrDbdZonesScalpBtcusdV2._detect_zones_cached`). A
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesScalpBtcusdV2` uses the
...
def _confluence_votes(
    closes: pd.Series, tick_volume: pd.Series, direction: Direction
) -> tuple[bool, bool, bool]:
    """RSI(14)/EMA(20,50)/volume-ratio confluence votes at the last bar,
    all computed on the entry timeframe (M1) closes -- distinct from the
    existing `_trend_direction` M5 gate. NaN (not enough warmup) degrades to
    "not confirmed" for that vote -- never raises, never skips the trade,
    since this whole path is additive-only."""
    ...
    rsi_ewm_kwargs = {
        "alpha": 1 / CONFLUENCE_RSI_PERIOD,
        "min_periods": CONFLUENCE_RSI_PERIOD,
        "adjust": False,
    }
    avg_gain = gain.ewm(**rsi_ewm_kwargs).mean()
    avg_loss = loss.ewm(**rsi_ewm_kwargs).mean()
    with np.errstate(divide="ignore", invalid="ignore"):
        rs = avg_gain / avg_loss
    rsi = 100 - 100 / (1 + rs)
    rsi_val = rsi.iloc[-1]
    rsi_ok = bool(pd.notna(rsi_val) and ((rsi_val > 50) if is_buy else (rsi_val < 50)))

    ema_fast = closes.ewm(span=CONFLUENCE_EMA_FAST, adjust=False).mean().iloc[-1]
    ema_slow = closes.ewm(span=CONFLUENCE_EMA_SLOW, adjust=False).mean().iloc[-1]
    ema_ok = bool(
        pd.notna(ema_fast)
        and pd.notna(ema_slow)
        and ((ema_fast > ema_slow) if is_buy else (ema_fast < ema_slow))
    )

    vol_avg20 = tick_volume.rolling(20).mean().iloc[-1]
    last_vol = tick_volume.iloc[-1]
    vol_ok = bool(pd.notna(vol_avg20) and vol_avg20 > 0 and last_vol > vol_avg20)

    return rsi_ok, ema_ok, vol_ok


class RbrDbdZonesScalpBtcusdV2:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="rbr_dbd_zones_scalp_btcusd",
            version=2,
            symbols=("BTCUSD",),
            entry_timeframe="M1",
            ...
            params={
                ...
                "tp_mult_base": TP_MULT_BASE,
                "tp_mult_tier2": TP_MULT_TIER2,
                "tp_mult_tier3": TP_MULT_TIER3,
            },
        )
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_rbr_dbd_zones_scalp_btcusd_v2.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
    ...
    def evaluate(self, ctx: MarketContext) -> Signal | None:
        ...
        # Additive-only confluence: extends tp_points for higher-conviction
        # setups, never shrinks it below v1's own value (tp_mult_base=1.0),
        # so this can only make min_rr_floor easier to clear, never harder.
        closes_s = df["close"].iloc[-lookback:]
        volume_s = df["tick_volume"].iloc[-lookback:]
        rsi_ok, ema_ok, vol_ok = _confluence_votes(closes_s, volume_s, direction)
        votes = sum((rsi_ok, ema_ok, vol_ok))
        if votes == 3:
            tp_mult = params["tp_mult_tier3"]
        elif votes == 2:
            tp_mult = params["tp_mult_tier2"]
        else:
        ...
        reason = (
            ...
            f"confluence rsi={rsi_ok} ema={ema_ok} vol={vol_ok} ({votes}/3) -> tp_mult={tp_mult} "
