"""RBR/DBD/RBD/DBR zone-retest scalp strategy for BTCUSD (M1
...
point, not re-derived for BTCUSD's own volatility/tick behavior — validate
with a BTCUSD backtest before trusting them, and retune (particularly
...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesScalpBtcusd._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesScalpBtcusd._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    in `RbrDbdZonesScalpBtcusd._detect_zones_cached`). A from-scratch
...
def _build_zones_from_runs(
    ...
    both the full recompute and `RbrDbdZonesScalpBtcusd._detect_zones_cached`
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesScalpBtcusd` uses the
...
class RbrDbdZonesScalpBtcusd:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            ...
            params={
                ...
                # PF 1.89->1.86, 171->451 trades, +51%->+163%
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_rbr_dbd_zones_scalp_btcusd.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
