...
veto most of this strategy's setups.

v3: same entry rules as v1 (the version live when this was written — v2's
freshness/regime filters were archived, never activated). v1 fires far more
often than this batch's other bots, and its 429-trade backtest (XAUUSD
2026-05:2026-07, PF 0.63) showed many of those trades sitting in a losing
position for a while, riding the SL/TP round trip, while price had already
crossed back through VWAP and started stretching the other way — i.e. the
strategy's own next signal, on a later bar, was often the earliest reliable
"this reversion is over, or reversed" signal, arriving well before the ATR
stop or the 2.2R target. v3 adds `close_on_opposite_signal=True`: when a
fresh signal opposes an open position this bot placed (matched by order
magic — see `engine/application/trade_loop.py::_close_opposite_position`),
the engine closes that position immediately and lets the new signal open a
fresh one in the same pass, instead of waiting for SL/TP/time-stop. SL/TP
stay in place as a tail-risk backstop for the case where price runs hard
against the position with no opposite signal in between: `atr_mult` widened
1.0 -> 3.0 so that backstop sits well outside where a same-strategy flip
would normally have already closed the trade.
...
ATR_PERIOD = 14
ATR_MULT = 3.0  # v3: widened from 1.0 — now a tail-risk backstop, not the primary exit
