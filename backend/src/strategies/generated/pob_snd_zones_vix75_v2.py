...
v2 (backtest-driven refinement, VIX75 2026-06:2026-07 baseline PF 0.78):
continuation zones only (RBR/DBD — the SNRC1 tier; reversal-turn RBD/DBR
entries dragged the win rate down) and both confirmation TFs required
(min_confirmations 2). Matrix result: PF 1.21, win 34.2%, 76 trades. The
engulfing-only entry gate and H1 trend filter were tested and REJECTED
(PF 0.82/1.20) — the pin-bar/body entries carry real signal here.
"""
...
def _classify_bars(
    ...
    `PobSndZonesVix75._detect_zones_cached` — keep the two in sync."""
...
class PobSndZonesVix75:
    ...
    def _detect_zones_cached(
        ...
        call — proven by
        `tests/unit/strategies/test_pob_snd_zones_vix75_v2.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(window)
        valid_atr = atr.dropna()
        if valid_atr.empty:
