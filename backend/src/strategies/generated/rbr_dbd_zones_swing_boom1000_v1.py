"""RBR/DBR demand-zone swing strategy for Boom 1000 Index (M15 entries,
H1+H4 trend), long-only.

Same mechanics and same tuned recipe as `rbr_dbd_zones_swing_xauusd_v1.py`
(see that module for the full derivation), ported to Boom 1000 Index rather than
re-derived from scratch since all thresholds are ATR-relative:
  ...
  - Entry timeframe M15, trend confirmation requires BOTH H1 and H4.
  - SL = base rectangle height (floored at a small ATR multiple); TP =
    nearest unmitigated swing high above price, with a capped RR fallback.
  - Entry confirmation is engulfing-only, and the zone base is capped at
    `max_base_atr_mult` x ATR — both filters came out of the XAUUSD
    backtests (plain pin bar and wide/sloppy bases underperformed badly
    enough there to drop outright) and are carried over as a starting point
    for Boom 1000 Index.

These are the XAUUSD-tuned parameter values, not re-derived for Boom 1000 Index's own
volatility/tick behavior — validate with a Boom 1000 Index backtest before trusting
them, and retune (particularly `base_body_atr_mult`/`leg_travel_atr_mult`/
`max_base_atr_mult`) if the results don't transfer, the same way the XAUUSD
sibling was iterated three rounds against its own backtests.

v2: a first backtest (84 trades, XAUUSD-tuned params unchanged, 2026-01..2026-07)
came back PF 0.73, avg_r -0.19 — a real loser, not a marginal one. Breaking it out
by zone pattern found RBD_flip (a Rally-Base-Drop supply zone that later got busted
back up by a strong candle) was the entire story: 18 trades, 5.6% win, PF 0.17,
avg_r -0.60, while RBR/DBR/DBD_flip were only mildly negative (PF 0.81-0.92).
Boom 1000 Index's defining trait is periodic sharp vertical "boom" spikes rather
than a smooth stochastic walk — a rally that dropped into a base and then gets
blown through upward looks exactly like what a spike event produces, but a single
spike isn't a genuine trend shift, so buying its "flip" retest kept getting caught
in the post-spike reversion. RBD_flip demand entries are excluded here; RBR/DBR/
DBD_flip are kept since they were only mildly negative, not the actual drag.
...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesSwingBoom1000._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesSwingBoom1000._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    in `RbrDbdZonesSwingBoom1000._detect_zones_cached`). A from-scratch
...
def _build_zones_from_runs(
    ...
    both the full recompute and `RbrDbdZonesSwingBoom1000._detect_zones_cached`
    ...
    `rbr_dbd_zones_scalp_xauusd_v1.py` reference this pattern was replicated
    from."""
    n = len(closes)
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesSwingBoom1000` uses the
...
class RbrDbdZonesSwingBoom1000:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="rbr_dbd_zones_swing_boom1000",
            version=1,
            ...
            params={
                "atr_period": 14,
                # Per-symbol retune 2026-07-19 (2025-11:2026-07 sweep):
                # PF 0.62->1.33, only combo cluster above water
        ...
        # on the window's own M15 bar timestamps (int64 ns) rather than
        ...
        # adapted for a window that's raw M15 bars (no zone-TF resample
    ...
    def _detect_zones_cached(
        ...
        M15 bar stream every time (see `__init__`). `t_ns` is the window's
        bar timestamps (int64 ns), positionally aligned 1:1 with
        opens/highs/lows/closes, ascending. There is no zone-TF resample
        step here — the window is raw M15 bars — so every position's OHLC
        ...
        `rbr_dbd_zones_scalp_xauusd_v1.RbrDbdZonesScalpXauusd._detect_zones_cached`
        for the fully-annotated original this was replicated from, including
        why raw pre-merge runs — not post-merge output — must be cached: a
        run that's still last-in-frame can keep growing on a later call and
        flip from "too short to be its own leg" to "a leg", which would
        ...
        current window every call, same as the scalp reference.

        Bit-identical to `_detect_zones(opens, highs, lows, closes, atr,
        params)` on every call — proven by
        `tests/unit/strategies/test_rbr_dbd_zones_swing_boom1000.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
