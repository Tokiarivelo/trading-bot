...
Zone-detection params were retuned for VIX75 on 2026-07-19 via a 26-combo
sweep over 2026-06:2026-07 M1 history (the XAUUSD values scored PF 0.98
here): `base_body_atr_mult` 0.5->0.8 and `leg_travel_atr_mult` 1.0->0.8
lifted the same window to PF 1.75, 205 trades, +55% on 0.5%-risk sizing,
max DD 3.3%. The optimum sits on a stable ridge (0.75-0.9 body x 0.7-0.9
leg all score PF 1.5-1.9), not a spike — higher `base_body` keeps raising
PF but sheds trades. `max_base_atr_mult` 2.0 vs 3.0 made zero difference
(VIX75 bases never got that wide); left at 2.0.

Refinements over the straight XAUUSD port:

  - Trend filter: EMA(`trend_fast_period`) vs EMA(`trend_slow_period`) on
    `trend_timeframe` (M5, the confirmation TF). Demand-zone buys only in an
    uptrend, supply-zone sells only in a downtrend — mirrors the engine's
    own HTF-veto proxy so live signals stop being generated just to get
    vetoed. Skipped (not vetoed) while the TF lacks history.
  - The entry candle's engulfing direction is classified FIRST, then every
    fresh zone is scanned for one matching that direction. Previously only
    the single most recent fresh zone was considered — a fresh-but-
    misaligned zone silently masked a valid aligned one on the same bar and
...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesScalpVix75._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesScalpVix75._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    in `RbrDbdZonesScalpVix75._detect_zones_cached`). A from-scratch
...
def _build_zones_from_runs(
    ...
    both the full recompute and `RbrDbdZonesScalpVix75._detect_zones_cached`
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesScalpVix75` uses the
...
class RbrDbdZonesScalpVix75:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="rbr_dbd_zones_scalp_vix75",
            version=1,
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_rbr_dbd_zones_scalp_vix75.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
