"""RBR/DBR demand-zone swing strategy for Volatility 75 Index (M15 entries,
H1+H4 trend), long-only.

Same mechanics and same tuned recipe as `rbr_dbd_zones_swing_xauusd_v1.py`
(see that module for the full derivation), ported to VIX75 rather than
...
These are the XAUUSD-tuned parameter values, not re-derived for VIX75's own
volatility/tick behavior — validate with a VIX75 backtest before trusting
...
def _classify_bars(
    ...
    to keep it in sync with `RbrDbdZonesSwingVix75._detect_zones_cached`."""
    return np.where(
...
def _build_runs_from(classes: np.ndarray, start: int, stop: int | None = None) -> list[list[int]]:
    ...
    `RbrDbdZonesSwingVix75._detect_zones_cached`."""
...
def _coalesce_adjacent_runs(runs: list[list[int]]) -> list[list[int]]:
    ...
    in `RbrDbdZonesSwingVix75._detect_zones_cached`). A from-scratch
...
def _build_zones_from_runs(
    ...
    both the full recompute and `RbrDbdZonesSwingVix75._detect_zones_cached`
...
def _detect_zones(
    ...
    this is the stateless ground truth. `RbrDbdZonesSwingVix75` uses the
...
class RbrDbdZonesSwingVix75:
    def __init__(self) -> None:
        self.spec = StrategySpec(
            name="rbr_dbd_zones_swing_vix75",
            version=1,
            ...
            params={
                "atr_period": 14,
                # Per-symbol retune 2026-07-19 (2025-09:2026-07 sweep):
                # PF 1.40->1.48, 23->27 trades, max DD 3.9%->2.1%
    ...
    def _detect_zones_cached(
        ...
        `tests/unit/strategies/test_rbr_dbd_zones_swing_vix75.py::test_incremental_cache_matches_full_recompute_every_step`.
        """
        n = len(closes)
        valid_atr = atr.dropna()
        if valid_atr.empty:
