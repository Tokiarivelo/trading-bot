...
@router.post(
    ...
    description=(
        ...
        "under `max_open_positions`, and not blocked by the `max_trades_per_day_enabled` "
        "manual kill switch — the same caps that gate automated entries) and the "
...
@router.post(
    ...
    description=(
        ...
        "placement — `max_open_positions` describes open trades and the "
        "`max_trades_per_day_enabled` manual kill switch is re-checked properly "
        "when the order actually fills, since a resting order isn't a trade yet. "
        "In paper mode the fill is simulated on each M5 close; "
