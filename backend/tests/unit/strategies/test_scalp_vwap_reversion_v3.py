...
from src.strategies.generated.scalp_vwap_reversion_v1_v3 import ScalpVwapReversionV3
...
def test_sell_signal_on_upward_stretch_with_bearish_reversal_candle():
    signal = ScalpVwapReversionV3().evaluate(
...
def test_no_signal_without_meaningful_deviation():
    ...
    highs = [c + 0.3 for c in closes]
    lows = [c - 0.3 for c in closes]
    assert ScalpVwapReversionV3().evaluate(_ctx(opens, highs, lows, closes, volumes)) is None
...
def test_spec_covers_all_five_symbols_and_carries_no_confirmation_timeframes():
    spec = ScalpVwapReversionV3().spec
...
def test_spec_opts_into_close_on_opposite_signal():
    # v3's whole point: the engine closes this bot's open position on a
    # signal flip instead of waiting for SL/TP/time-stop.
    assert ScalpVwapReversionV3().spec.close_on_opposite_signal is True
