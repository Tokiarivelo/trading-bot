"""Unit tests for `rbr_dbd_zones_scalp_xauusd_noveto_v1.py` — forked from
`rbr_dbd_zones_scalp_xauusd_v1.py` (same zone-detection engine, same tests for
...
import src.strategies.generated.rbr_dbd_zones_scalp_xauusd_noveto_v1 as mod
...
PARAMS = mod.RbrDbdZonesScalpXauusdNoveto().spec.params
...
def test_spec_opts_out_of_engine_htf_veto():
    spec = mod.RbrDbdZonesScalpXauusdNoveto().spec
...
def test_evaluate_buys_demand_zone_retest():
    strategy = mod.RbrDbdZonesScalpXauusdNoveto()
    ctx = MarketContext(
        symbol="XAUUSD",
        candles={
...
def test_evaluate_none_without_retest():
    ...
    assert mod.RbrDbdZonesScalpXauusdNoveto().evaluate(ctx) is None
...
def test_evaluate_buys_when_m5_trend_aligned():
    ...
    signal = mod.RbrDbdZonesScalpXauusdNoveto().evaluate(ctx)
    assert signal is not None
    assert signal.direction == Direction.BUY
