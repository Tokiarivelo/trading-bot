"""Unit tests for `rbr_dbd_zones_swing_vix75_v1.py` — direct port of
`rbr_dbd_zones_swing_xauusd_v1.py` to Volatility 75 Index; same mechanics,
...
def test_evaluate_buys_demand_zone_retest():
    strategy = mod.RbrDbdZonesSwingVix75()
    ctx = MarketContext(
...
def test_evaluate_none_without_htf_confirmation():
    ...
    assert mod.RbrDbdZonesSwingVix75().evaluate(ctx) is None
