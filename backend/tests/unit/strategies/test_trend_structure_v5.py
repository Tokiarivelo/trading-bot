...
#
# `TrendStructureV5._detect_zones_cached` is a timestamp-keyed incremental
...
# the swing/structure check and RSI gate entries), so these tests call
...
def test_incremental_cache_matches_full_recompute_every_step() -> None:
    ...
    single step assert the incremental cache (`TrendStructureV5._detect_
    ...
    params = mod.TrendStructureV5().spec.params
    incremental = mod.TrendStructureV5()
