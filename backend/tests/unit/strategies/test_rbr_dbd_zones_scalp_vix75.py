"""Unit tests for `rbr_dbd_zones_scalp_vix75_v1.py` — direct port of
`rbr_dbd_zones_scalp_xauusd_v1.py` to Volatility 75 Index; same mechanics,
...
import src.strategies.generated.rbr_dbd_zones_scalp_vix75_v1 as mod
...
PARAMS = mod.RbrDbdZonesScalpVix75().spec.params
...
def test_evaluate_buys_demand_zone_retest():
    strategy = mod.RbrDbdZonesScalpVix75()
    ctx = MarketContext(
...
def test_evaluate_none_without_retest():
    ...
    assert mod.RbrDbdZonesScalpVix75().evaluate(ctx) is None
...
def test_evaluate_buys_when_m5_trend_aligned():
    ...
    signal = mod.RbrDbdZonesScalpVix75().evaluate(ctx)
    assert signal is not None
    assert signal.direction == Direction.BUY
...
#
# `RbrDbdZonesScalpVix75._detect_zones_cached` is a timestamp-keyed
...
def test_incremental_cache_matches_full_recompute_every_step() -> None:
    ...
    (`RbrDbdZonesScalpVix75._detect_zones_cached`, called on one persistent
    ...
    params = mod.RbrDbdZonesScalpVix75().spec.params
    incremental = mod.RbrDbdZonesScalpVix75()
...
def test_incremental_cache_reprocesses_far_fewer_bars_than_full_recompute() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        bars_walked.clear()
        cold = mod.RbrDbdZonesScalpVix75()
