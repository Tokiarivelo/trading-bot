"""Unit tests for `rbr_dbd_zones_swing_xauusd_v1.py` — M15 entries, H1+H4
trend confirmation, long-only RBR/DBR demand-zone swing strategy."""
...
import src.strategies.generated.rbr_dbd_zones_swing_xauusd_v1 as mod
...
PARAMS = mod.RbrDbdZonesSwingXauusd().spec.params
...
def test_detect_zones_finds_dbr_demand_reversal():
    """DBR: drop-in leg, base, then a rally-out leg — a demand zone marking
    a reversal out of a downtrend."""
    bars = _flat(34)
    i = len(bars)
...
def test_evaluate_buys_demand_zone_retest():
    strategy = mod.RbrDbdZonesSwingXauusd()
    ctx = MarketContext(
        symbol="XAUUSD",
        candles={
...
def test_evaluate_ignores_supply_zone_retest_long_only():
    """A textbook DBD supply retest that would be a valid sell on the scalp
    variant must never signal here — this variant is long-only."""
    strategy = mod.RbrDbdZonesSwingXauusd()
    ctx = MarketContext(
        symbol="XAUUSD",
        candles={
...
def test_evaluate_none_without_htf_confirmation():
    ...
    assert mod.RbrDbdZonesSwingXauusd().evaluate(ctx) is None
...
def test_evaluate_buys_when_h1_trend_aligned():
    ...
    signal = mod.RbrDbdZonesSwingXauusd().evaluate(ctx)
    assert signal is not None
    assert signal.direction == Direction.BUY
...
# `RbrDbdZonesSwingXauusd._detect_zones_cached` (evaluate()'s per-instance,
...
def test_incremental_cache_matches_full_recompute_every_step() -> None:
    ...
    (`RbrDbdZonesSwingXauusd._detect_zones_cached`, called on one persistent
    ...
    params = mod.RbrDbdZonesSwingXauusd().spec.params
    incremental = mod.RbrDbdZonesSwingXauusd()
...
def test_incremental_cache_reprocesses_far_fewer_bars_than_full_recompute() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        bars_walked.clear()
        cold = mod.RbrDbdZonesSwingXauusd()
