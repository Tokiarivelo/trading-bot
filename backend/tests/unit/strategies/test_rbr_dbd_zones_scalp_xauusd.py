"""Unit tests for `rbr_dbd_zones_scalp_xauusd_v1.py` — M1/M5 RBR/DBD/RBD/DBR
zone-retest scalp strategy, both directions, with zone-flip support."""
...
import src.strategies.generated.rbr_dbd_zones_scalp_xauusd_v1 as mod
...
def _flat(n: int) -> list[dict]:
    """Identical flat bars → ATR(14) settles around 1.2, matching the same
    base/leg thresholds used by pob_snd_zones_vix75_v1's tests."""
...
PARAMS = mod.RbrDbdZonesScalpXauusd().spec.params
...
def test_detect_zones_flips_polarity_on_strong_break():
    """A demand (RBR) zone whose far side is later closed through by a
    strong candle should register a second, flipped SUPPLY zone over the
    same rectangle — 'if zone break with a long candle ... it becomes a
    sell/buy zone.'"""
    bars = _flat(34)
    i = len(bars)
    bars.append(_bar(i, 100.0, 104.2, 100.0, 104.0))  # rally in
...
def _pattern_tail(rally: bool) -> list[dict]:
    """RBR demand zone at [103.6, 104.4] (or DBD supply mirror when
    rally=False) followed by a multi-bar drift and a retest bar that both
    tags the band and closes as the confirming engulfing candle."""
    if rally:
        return [
...
def _padded_bars(tail: list[dict]) -> pd.DataFrame:
    """Enough flat warmup bars ahead of `tail` so evaluate()'s min-history
    guard (zone_lookback_bars=220) is satisfied, re-timestamped so the whole
...
def test_evaluate_buys_demand_zone_retest():
    strategy = mod.RbrDbdZonesScalpXauusd()
    ctx = MarketContext(
        symbol="XAUUSD",
        candles={
...
def test_evaluate_none_without_retest():
    ...
    assert mod.RbrDbdZonesScalpXauusd().evaluate(ctx) is None
...
def test_evaluate_buys_when_m5_trend_aligned():
    ...
    signal = mod.RbrDbdZonesScalpXauusd().evaluate(ctx)
    assert signal is not None
    assert signal.direction == Direction.BUY
...
#
# `RbrDbdZonesScalpXauusd._detect_zones_cached` is a timestamp-keyed
...
def test_incremental_cache_matches_full_recompute_every_step() -> None:
    ...
    (`RbrDbdZonesScalpXauusd._detect_zones_cached`, called on one persistent
    ...
    params = mod.RbrDbdZonesScalpXauusd().spec.params
    incremental = mod.RbrDbdZonesScalpXauusd()
...
def test_incremental_cache_reprocesses_far_fewer_bars_than_full_recompute() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        bars_walked.clear()
        cold = mod.RbrDbdZonesScalpXauusd()
