...
#
# `TrendStructureV4._detect_zones_cached` is a timestamp-keyed incremental
...
# `_detect_zones` here is byte-identical to `trend_structure_v3_v1.py` /
...
def test_incremental_cache_matches_full_recompute_every_step() -> None:
    ...
    single step assert the incremental cache (`TrendStructureV4._detect_
    ...
    params = mod.TrendStructureV4().spec.params
    incremental = mod.TrendStructureV4()
