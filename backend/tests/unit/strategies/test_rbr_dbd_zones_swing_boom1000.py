"""Unit tests for `rbr_dbd_zones_swing_boom1000_v1.py` — direct port of
`rbr_dbd_zones_swing_xauusd_v1.py` to Boom 1000 Index; same mechanics,
...
import src.strategies.generated.rbr_dbd_zones_swing_boom1000_v1 as mod
...
PARAMS = mod.RbrDbdZonesSwingBoom1000().spec.params
...
def test_evaluate_buys_demand_zone_retest():
    strategy = mod.RbrDbdZonesSwingBoom1000()
    ctx = MarketContext(
...
def test_evaluate_none_without_htf_confirmation():
    ...
    assert mod.RbrDbdZonesSwingBoom1000().evaluate(ctx) is None
...
def test_evaluate_buys_when_h1_trend_aligned():
    ...
    signal = mod.RbrDbdZonesSwingBoom1000().evaluate(ctx)
    assert signal is not None
    assert signal.direction == Direction.BUY
...
# `RbrDbdZonesSwingBoom1000._detect_zones_cached` (evaluate()'s per-instance,
...
def test_incremental_cache_matches_full_recompute_every_step() -> None:
    ...
    (`RbrDbdZonesSwingBoom1000._detect_zones_cached`, called on one persistent
    ...
    params = mod.RbrDbdZonesSwingBoom1000().spec.params
    incremental = mod.RbrDbdZonesSwingBoom1000()
...
def test_incremental_cache_reprocesses_far_fewer_bars_than_full_recompute() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        bars_walked.clear()
        cold = mod.RbrDbdZonesSwingBoom1000()
