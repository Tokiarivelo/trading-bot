"""PoB QM2P (Quasimodo + 2-point head trendline) — p.120 of "The Property of
Bystra".

Same H -> L -> HH -> LL (or mirror) Quasimodo structure as QMR, plus a
trendline through the two points framing the head (the neckline point and
the head extreme) — p.120's "2 points of trendline on top of QM's head" —
that the neckline retest must also touch/break, the extra confluence that
upgrades a plain QMR reversal. Draws the neckline (`neckline`), the head
trendline (`head_trendline`), the head/extreme swing points (`head_marker`/
`extreme_marker`), and the retest entry (`entry_marker_up`/
`entry_marker_down`). Plain QMR (no head trendline) is its own, separate
indicator.

Ported from `_find_qm_structure`/`_qm_head_trendline_touches`/
`_classify_pattern` in
...
class PobQm2pIndicator:
    def compute(self, candles: pd.DataFrame, params: dict) -> dict[str, list[float | None]]:
        ...
        head_trendline: list[float | None] = [None] * n
        ...
        for k in range(3, len(raw_points)):
            ...
            if pd.isna(atr_val) or atr_val <= 0:
                continue
            # QM2P = QMR + the neckline retest must ALSO touch a trendline
            # through the two points framing the head (p.120) — the plain
            # QMR indicator covers the case without this extra confluence.
            line = _fit_trendline(
                [(qm["neckline_idx"], qm["neckline_price"]), (qm["head_idx"], qm["head_price"])]
            )
            if line is None:
                continue
            trend_tolerance = atr_val * p["trendline_tolerance_atr_mult"]
            if not _touches_trendline(df, *line, extreme_idx, trend_tolerance):
                continue
            ...
            for i in range(extreme_idx + 1, horizon + 1):
                if abs(df["close"].iloc[i] - qm["neckline_price"]) > tolerance:
                    continue
                if not _touches_trendline(df, *line, i, trend_tolerance):
                    continue
                ...
                for t in range(qm["neckline_idx"], i + 1):
                    ...
                    head_trendline[t] = line[0] * t + line[1]
