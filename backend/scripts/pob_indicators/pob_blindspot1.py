"""PoB Blindspot 1 — pp.139-151 of "The Property of Bystra".

A failed engulfing candle, followed later by a candle that closes beyond it
(the "significant break"), entry at a retest of that break level. The
author explicitly distrusts this family ("not all types of swaps can be
trusted") — this indicator only draws where the geometry occurred; it
carries no confidence/risk gating of its own (a strategy concern, not a
chart-annotation one). Draws the failed-engulfing/break candle
(`break_marker`) and the retest entry (`entry_marker_up`/
`entry_marker_down`). Blindspot 2 (same shape + a trendline confluence at
the retest) is its own, separate indicator.
...
def _find_blindspot_at(
    ...
    for i in range(j + 1, horizon + 1):
        ...
        has_trendline = False
        if len(same_side) >= 2:
            line = _fit_trendline([(pt[0], pt[1]) for pt in same_side[-2:]])
            if line:
                trend_tol = atr_val * params["trendline_tolerance_atr_mult"]
                has_trendline = _touches_trendline(df, *line, i, trend_tol)
...
class PobBlindspot1Indicator:
    def compute(self, candles: pd.DataFrame, params: dict) -> dict[str, list[float | None]]:
        ...
        for j in range(1, n - 1):
            ...
            if found is None or found["has_trendline"]:
                continue  # that's Blindspot 2's variant
            i = found["entry_idx"]
