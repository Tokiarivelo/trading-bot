"""PoB SNRC1 (SnR Continuation 1) — pp.44-45 of "The Property of Bystra".

Continuation setup: a tight compression base after an impulse leg, then a
breakout candle in the SAME direction as the leg before the base (RBR after
a rally, DBD after a drop). Draws the base zone (`zone_high`/`zone_low`) and
an arrow at the breakout/entry bar (`entry_marker_up`/`entry_marker_down`).
...
class PobSnrc1Indicator:
    def compute(self, candles: pd.DataFrame, params: dict) -> dict[str, list[float | None]]:
        ...
        for i in range(min_bars, n):
            ...
            # SNRC1 = continuation: breakout resumes the same direction as
            # the impulse leg before the base (RBR after a rally, DBD after
            # a drop). SNRC2 is the opposite-direction sibling — its own
            # indicator.
            is_snrc1 = (leg_before == "up" and breakout_up) or (
                leg_before == "down" and not breakout_up
            )
            if not is_snrc1:
                continue
