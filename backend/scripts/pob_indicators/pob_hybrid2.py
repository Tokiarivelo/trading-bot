"""PoB Hybrid 2 — pp.35-43 of "The Property of Bystra".

Same trendline-break + retest as Hybrid 1, but with the author's own
"upgrade": a second, more recent trendline on the opposite side (a channel)
that the retest bar also touches. Draws the primary trendline (`trendline`),
the channel trendline (`channel_trendline`), the origin/trend points
(`trend_marker`), and the break/retest entry (`entry_marker_up`/
`entry_marker_down`). Plain Hybrid 1 (no channel confluence) is its own,
separate indicator.
...
def _find_hybrid_at(
    ...
    for i in range(last_pt_idx + 1, horizon + 1):
        ...
        if len(opposite_pool) >= 2:
            candidate_channel = _fit_trendline([(pt[0], pt[1]) for pt in opposite_pool[-2:]])
            if candidate_channel and _touches_trendline(df, *candidate_channel, i, tolerance):
                is_hybrid2 = True
                channel = candidate_channel
                channel_points = opposite_pool[-2:]
...
class PobHybrid2Indicator:
    def compute(self, candles: pd.DataFrame, params: dict) -> dict[str, list[float | None]]:
        ...
        channel_trendline: list[float | None] = [None] * n
        ...
        if n == 0:
            return {
                "trendline": trendline,
                "channel_trendline": channel_trendline,
        ...
        for k in range(len(raw_points)):
            ...
            if found is None or not found["is_hybrid2"]:
                continue
            ...
            channel_start = found["channel_points"][0][0]
            for t in range(channel_start, i + 1):
                channel_trendline[t] = channel[0] * t + channel[1]
